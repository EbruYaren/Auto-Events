...
def main():
    ...
    for pair in get_date_pairs(start_date, end_date):
        start, end = pair
        run(start, end, domains, courier_ids, no_copy_param)
...
def run(start_date: str, end_date: str, domains: list, courier_ids: list, no_copy_param: bool):
    ...
    for domain_type in domain_types:
        threads[domain_type] = Thread(target=fetch_orders_and_process,
                                      args=[start_date, end_date, start_time, courier_ids, domains, domain_type, no_copy_param])
...
def fetch_orders_and_process(start_date, end_date, start_time, courier_ids: [], domains, domain_type: int, no_copy_param: bool):
    ...
    for chunk_df in orders_fetch:
        ...
        if order_index == (orders_len // chunk_size):
            last_chunk = True
        get_routes_and_process(chunk_df, domains, domain_type, start_date, end_date, start_time, last_chunk, no_copy_param)
    if orders_len > 0:
        print('Market Orders fetched!')


def get_routes_and_process(chunk_df, domains, domain_type, start_date, end_date, start_time, last_chunk, no_copy_param: bool):
    ...
    if chunk_df.size > 0:
            ...
            if domain_type in (2, 6):
                ...
                for domain in ['depart_from_merchant', 'depart_from_courier_warehouse']:
                    processed_depart_orders = depart_main(chunk_df, domain_type, domain, merged_df, start_time,
                                                          last_chunk, no_copy_param)
                    total_processed_routes_for_depart += (processed_depart_orders or 0)
            else:
                processed_depart_orders = depart_main(chunk_df, domain_type, '', merged_df, start_time, last_chunk, no_copy_param)
                total_processed_routes_for_depart += (processed_depart_orders or 0)
        ...
        if 'reach' in domains and domain_type not in (2, 6):
            processed_reach_orders = reach_main(chunk_df, domain_type, merged_df, start_time, last_chunk, no_copy_param).get('routes')
            total_processed_routes_for_reach += processed_reach_orders
            reach_predictions = reach_main(chunk_df, domain_type, merged_df, start_time, last_chunk, no_copy_param).get('preds')
        ...
        if 'depart_from_client' in domains and domain_type not in (2, 6):
            ...
            depart_from_client_dict = depart_from_client_main(chunk_df, routes_df, reach_predictions,
                                                              domain_type, merged_df, start_time, last_chunk, no_copy_param)
        ...
        if 'deliver' in domains and domain_type not in (2, 6):
            ...
            result_dict = deliver_main(reach_predictions, depart_from_client_predictions, domain_type, start_time, last_chunk, no_copy_param)
        ...
        if 'reach_to_merchant' in domains and domain_type in (2, 6):
            processed_reach_to_merchant_orders = reach_to_merchant_main(chunk_df, domain_type, merged_df, start_time, last_chunk, no_copy_param)
...
def reach_main(chunk_df: pd.DataFrame, domain_type: int, merged_df: pd.DataFrame, start_time: str, last_chunk: bool, no_copy_param: bool):
    ...
    if domain_type in (1, 3):
        ...
        writer.write()
        if last_chunk == True and no_copy_param == False:
...
def reach_to_merchant_main(chunk_df: pd.DataFrame, domain_type, merged_df: pd.DataFrame, start_time: str, last_chunk: bool, no_copy_param: bool):
...
def depart_main(chunk_df: pd.DataFrame, domain_type: int, domain: str, merged_df: pd.DataFrame, start_time: str, last_chunk: bool, no_copy_param: bool):
    order_ids = pd.DataFrame(chunk_df['_id_oid'], columns=['_id_oid'])
...
def depart_from_client_main(chunk_df: pd.DataFrame, routes_df: pd.DataFrame, reach_predictions_df: pd.DataFrame,
                            domain_type: int, merged_df: pd.DataFrame, start_time: str, last_chunk: bool, no_copy_param: bool):
    order_ids = pd.DataFrame(chunk_df['_id_oid'], columns=['_id_oid'])
...
def deliver_main(reach_df: pd.DataFrame, depart_from_client_df: pd.DataFrame, domain_type: int, start_time: str, last_chunk: bool, no_copy_param: bool):
