...
class SinglePredictor(abc.ABC):
    ...
    @staticmethod
    def process_depart_from_warehouse_after_predict(df: pd.DataFrame, domain_type: int, orders_df:pd.DataFrame,
                                                    max_distance_to_warehouse: float):
        ...
        true_preds = df[(df['predictions'] == 1) &
                        (df['prev_distance_to_warehouse'] < max_distance_to_warehouse)]
        ...
        labeled_times = SinglePredictor.get_predicted_depart_rows(true_preds, df, domain_type=domain_type)
        ...
        if domain_type not in (2, 6, 4):
            return SinglePredictor.get_unpredicted_batched_orders(orders_df, labeled_times).reset_index(drop=True)
        else:
    ...
    @staticmethod
    def get_unpredicted_batched_orders(orders_df, predictions):
        # Getting processed data before prediction process and group by _id_oid to eliminate multiple routes
        data = orders_df.groupby(['_id_oid']).max().reset_index()
...
class DepartBulkPredictor:
    ...
    def predict_in_bulk(self):
        if self.__processed_data.empty is False:
            ...
            result = SinglePredictor.process_depart_from_warehouse_after_predict(df, domain_type=self.__domain_type,
                                                                                 orders_df=self.__orders_df,
                                                                                 max_distance_to_warehouse=self.__max_distance_to_warehouse)
...
class NewDepartModelPredictor:
    def __init__(self, processed_data: pd.DataFrame, max_distance_to_warehouse:float, orders_df: pd.DataFrame,
                 domain_type: int):
        self.__processed_data = processed_data
        self.__max_distance_to_warehouse = max_distance_to_warehouse
        self.__orders_df = orders_df
    ...
    def predict(self, catboost_model=catboost_model):
        if self.__processed_data.empty is False:
            df = self.__processed_data.copy()
            predictors = ['smooth_speed_to_warehouse', 'smooth_speed_to_prev_event', 'dbw_warehouse_log', 'is_car']
            preds = catboost_model.predict(df[predictors])

            df['predictions'] = list(map(lambda x: True if x=='True' else 1, preds))
